...
function SettingsPanel(sdk: ClientSDK) {
    ...
    const handleProviderChange = (e: any) => {
        settingsState.settings.aiProvider = e.target.value as AIProvider;
        sdk.refresh();
    }
    ...
    return (
        <div className="p-6 max-w-2xl mx-auto bg-white rounded-lg shadow">
            ...
            <div className="space-y-6">
                <div>
                    <label className="block text-lg font-semibold mb-2">AI Provider</label>
                    <select name="aiProvider" value={settingsState.settings.aiProvider} onChange={handleProviderChange} className="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500">
                        <option value="gemini">Gemini</option>
                        <option value="openai">OpenAI</option>
                    </select>
                </div>

                {settingsState.settings.aiProvider === 'gemini' && (
                    <>
                        <div>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                value={settingsState.settings.geminiModelName}
                                onChange={handleInputChange}
                ...
                {settingsState.settings.aiProvider === 'openai' && (
                    <>
                        <div>
                            <label className="block text-lg font-semibold mb-2">OpenAI API Key</label>
                            <input
                                type="text"
                                name="openaiApiKey"
                                value={settingsState.settings.openaiApiKey}
                                onChange={handleInputChange}
                        ...
                        <div>
                            <label className="block text-lg font-semibold mb-2">OpenAI Model Name</label>
                            <input
                                type="text"
                                name="openaiModelName"
                                value={settingsState.settings.openaiModelName}
                                onChange={handleInputChange}
